...
export function initCreatorDock(): void {
  ...
  let files: DraftFile[] = [], posts: ScheduledPost[] = [], createdAt = Date.now(), saveTimer = 0, restoreVersion = 0;
  ...
  const snapshot = () => ({
    metadata: { id: activeDraftId, type: format, caption, createdAt, updatedAt: Date.now() },
    media: files.map(({ id, file }) => ({ id, name: file.name, type: file.type, size: file.size, blob: file })),
  });
  const persist = (owner = accountId, draft = snapshot()): Promise<void> => {
    if (!owner) return Promise.resolve();
    const operation = draftMutation.catch(() => undefined).then(() => saveDraft(owner, draft.metadata, draft.media));
    draftMutation = operation;
    return operation;
  };
  const queueSave = () => {
    window.clearTimeout(saveTimer);
    const owner = accountId, draft = snapshot();
    saveTimer = window.setTimeout(() => void persist(owner, draft).catch((error: unknown) => { status = error instanceof Error ? error.message : 'Could not save this draft.'; render(); }), 250);
  };
  const resetDraft = () => { format = 'reel'; caption = ''; files = []; createdAt = Date.now(); };
  const restore = async () => {
    const version = ++restoreVersion;
    const auth = await send<AuthState>(MESSAGE_TYPES.GET_AUTH);
    if (version !== restoreVersion) return;
    const owner = auth.isLoggedIn ? auth.userId : null;
    if (owner !== accountId) { window.clearTimeout(saveTimer); resetDraft(); status = ''; }
    accountId = owner;
    if (!owner) return;
    const stored = await getDraft(owner, activeDraftId);
    if (version !== restoreVersion || accountId !== owner || !stored) return;
  ...
  function bind(): void {
    ...
    const mediaInput = dock.querySelector<HTMLInputElement>('#im-file-input'); mediaInput?.addEventListener('change', () => { const selected = [...(mediaInput.files ?? [])]; const next = [...files]; for (const file of selected) { const duplicate = next.some((item) => item.file.name === file.name && item.file.size === file.size && item.file.type === file.type && item.file.lastModified === file.lastModified); if (!duplicate) next.push({ id: crypto.randomUUID(), file }); } if (next.length > 20) status = 'Select no more than 20 files.'; else if (selected.some((file) => !allowed.has(file.type) || file.size > maxFileBytes)) status = 'Choose supported files under 250 MB.'; else { files = next; status = selected.length ? 'Draft media saved locally.' : status; queueSave(); } render(); });
    ...
    dock.querySelector('[data-clear]')?.addEventListener('click', () => void clearDraft().catch(showError));
    ...
    dock.querySelector('[data-schedule]')?.addEventListener('click', () => void scheduleReminder().catch(showError)); dock.querySelector('[data-handoff]')?.addEventListener('click', () => void handoff().catch(showError));
    dock.querySelectorAll<HTMLElement>('[data-use]').forEach((element) => element.addEventListener('click', () => void openScheduled(element.dataset.use).catch(showError)));
    dock.querySelectorAll<HTMLElement>('[data-delete]').forEach((element) => element.addEventListener('click', () => void remove(element.dataset.delete).catch(showError)));
  }
  function showError(error: unknown): void { status = error instanceof Error ? error.message : 'The operation could not be completed.'; render(); }
  async function clearDraft(): Promise<void> { window.clearTimeout(saveTimer); resetDraft(); status = ''; if (accountId) { const owner = accountId; const operation = draftMutation.catch(() => undefined).then(() => deleteDraft(owner, activeDraftId)); draftMutation = operation; await operation; } render(); }
  async function scheduleReminder(): Promise<void> { if (scheduleInFlight) return; if (!caption && !files.length) { status = 'Add media or write a caption before scheduling.'; render(); return; } if (!accountId) { status = 'Log in to Instagram before scheduling.'; render(); return; } scheduleInFlight = true; render(); try { const owner = accountId, draftId = `scheduled-${crypto.randomUUID()}`, scheduledDraft = { id: draftId, type: format, caption, createdAt: Date.now(), updatedAt: Date.now() }; await saveDraft(owner, scheduledDraft, files.map(({ id, file }) => ({ id, name: file.name, type: file.type, size: file.size, blob: file }))); const response = await send<{ success: boolean; error?: string }>(MESSAGE_TYPES.SCHEDULE_POST, { accountId: owner, type: format, caption, scheduledAt: new Date(`${date}T${time}:00`).getTime(), mediaName: files.length > 1 ? `${files.length} files · ${files[0].file.name}` : files[0]?.file.name ?? null, mediaType: files.length > 1 ? 'carousel' : files[0]?.file.type ?? null, draftId }); if (!response.success) { await deleteDraft(owner, draftId); throw new Error(response.error ?? 'Could not schedule reminder.'); } status = 'Reminder scheduled with locally saved media.'; await refresh(); tab = 'queue'; } finally { scheduleInFlight = false; render(); } }
  async function openScheduled(id?: string): Promise<void> { const post = posts.find((item) => item.id === id); if (!post) return; const owner = accountId; format = post.type; caption = post.caption; files = []; if (owner && post.accountId === owner && post.draftId) { const stored = await getDraft(owner, post.draftId); if (accountId !== owner) return; if (stored) files = stored.media.map((item) => ({ id: item.id, file: new File([item.blob], item.name, { type: item.type, lastModified: stored.updatedAt }) })); } tab = 'draft'; status = files.length ? 'Scheduled draft opened with locally saved media.' : 'Scheduled draft opened without saved media.'; render(); }
  async function remove(id?: string): Promise<void> { if (!id) return; const owner = accountId, post = posts.find((item) => item.id === id); if (!owner || !post || post.accountId !== owner) throw new Error('Scheduled reminder not found for this account.'); const response = await send<{ success: boolean; error?: string }>(MESSAGE_TYPES.DELETE_SCHEDULED_POST, { id }); if (!response.success) throw new Error(response.error ?? 'Could not delete scheduled reminder.'); if (post.draftId) await deleteDraft(owner, post.draftId); await refresh(); status = 'Scheduled reminder deleted.'; render(); }
